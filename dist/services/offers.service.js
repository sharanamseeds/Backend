...
const getOfferList = ({ query, }) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const lang_code = query.lang_code;
        let { limit, page, pagination = true, sortBy = "createdAt", sortOrder = "asc", product_id, category_id, offer_code, offer_name, offer_type, is_active, search, } = query;
        ...
        if (product_id) {
            orConditions.push({ is_active: true }, { product_specified: true }, { products: { $in: [new mongoose.Types.ObjectId(product_id)] } });
        }
        if (category_id) {
            orConditions.push({ is_active: true }, { category_specified: true }, { categories: { $in: [new mongoose.Types.ObjectId(category_id)] } });
        }
        ...
        if (!pagination) {
            const offerDoc = yield Offer.aggregate([
                { $match: filterQuery },
...
const getCustomerOfferList = ({ query, }) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const lang_code = query.lang_code;
        let { limit, page, pagination = true, sortBy = "createdAt", sortOrder = "asc", product_id, category_id, offer_code, offer_name, offer_type, search, } = query;
        if (typeof limit === "string") {
            limit = Number(limit);
        }
        if (!limit || isNaN(limit)) {
        ...
        let filterQuery = {};
        let orConditions = [{ is_active: true }];
        if (product_id) {
            orConditions.push({ is_active: true }, { product_specified: true }, { products: { $in: [new mongoose.Types.ObjectId(product_id)] } });
        }
        if (category_id) {
...
const getOffer = ({ offerId, query, }) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const lang_code = query.lang_code;
...
const addOffer = ({ requestUser, req, }) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        ...
        if (Array.isArray(image) && image.length > 0) {
            const savedFile = yield createDocument({
                ...
                documentPath: masterConfig.fileStystem.folderPaths.OFFERS +
                    offerData._id +
                    "/" +
        ...
        const offer = new Offer(Object.assign({}, offerData));
        let offerDoc = yield offer.save();
        return offerDoc;
    }
    catch (error) {
        throw error;
...
const updateOffer = ({ offerId, requestUser, req, }) => __awaiter(void 0, void 0, void 0, function* () {
    var _b, _c;
    try {
        const lang_code = req.query.lang_code;
        let offerDoc = yield Offer.findById(offerId);
        let bodyData = {};
...
const deleteOffer = ({ offerId }) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        yield Offer.findByIdAndDelete(offerId);
